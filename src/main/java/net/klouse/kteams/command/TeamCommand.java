...
import net.klouse.kteams.manager.InviteData;
...
public final class TeamCommand implements CommandExecutor, TabCompleter {
    ...
    public TeamCommand(KTeams plugin, TeamManager manager, TeamGUI gui) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "rename" -> {
                ...
                if (!player.hasPermission("kteams.rename") && !player.hasPermission("kteams.admin")) {
            ...
            case "reload" -> {
                if (!player.hasPermission("kteams.reload") && !player.hasPermission("kteams.admin")) {
    ...
    private void handleAdmin(Player player, String[] args) {
        if (!player.hasPermission("kteams.admin")) {
            player.sendMessage(ChatUtil.message(plugin, "admin-no-permission"));
            SoundUtil.play(player, plugin.getConfig().getString("sounds.error"), 1f, 1f);
            return;
        }
